...
// Railway sirve Express detrás de un proxy HTTPS.
...
async function enviarCorreoPedido(pedido, cliente) {
  ...
  const info = await transporter.sendMail({
    from: `ALTER EGO <${process.env.SMTP_USER}>`,
    to: STORE_EMAIL,
...
app.get('/', (_req, res) => {
  res.json({ ok: true, mensaje: 'Backend ALTER EGO funcionando', entorno: process.env.NODE_ENV || 'development' });
});

app.get('/api/health', (_req, res) => {
  res.status(200).json({
    ok: true,
    servicio: 'ALTER EGO API',
...
app.post('/api/pedidos', verificarToken, async (req, res) => {
  ...
  try { await enviarCorreoPedido(pedido, clienteCompleto); } catch (e) { console.error('Correo no enviado:', e.message); }
...
const server = app.listen(PORT, HOST, () => {
  console.log(`Backend ALTER EGO activo en http://${HOST}:${PORT}`);
});
