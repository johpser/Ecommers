const ES_ENTORNO_LOCAL =
    window.location.hostname === 'localhost' ||
    window.location.hostname === '127.0.0.1';

const API_LOCAL = 'http://localhost:3000/api';
const API_RAILWAY = 'https://ecommerce-production-a450.up.railway.app/api';

// En producción se fuerza Railway. De esta manera un valor antiguo de
// localhost guardado en sessionStorage no puede romper GitHub Pages.
const API_PERSONALIZADA = String(
    window.ALTEREGO_API_URL ||
    sessionStorage.getItem('ALTEREGO_API_URL') ||
    ''
).trim();

const API_BACKEND_CONFIGURADO = (
    ES_ENTORNO_LOCAL
        ? (API_PERSONALIZADA || API_LOCAL)
        : API_RAILWAY
).replace(/\/+$/, '');

// Limpia configuraciones antiguas que apuntaban a localhost cuando la web
// ya está publicada.
if (!ES_ENTORNO_LOCAL && /localhost|127\.0\.0\.1/i.test(API_PERSONALIZADA)) {
